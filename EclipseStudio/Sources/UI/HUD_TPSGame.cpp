...
static void DrawMenus()
{
	...
	else
	{
		...
		if(hudMain)
		{
			...
			if(InputMappingMngr->wasReleased(r3dInputMappingMngr::KS_CHAT_CHANNEL4)) //ViruZ Group
		...
		if(hudMain && !hudMain->isChatInputActive())
		{
			...
			if(showPlayerList)
			{
				if (!hudMain->isPlayersListVisible())
				{
				   ...
				   // - viruz hudMain->addPlayerToList(index++, CGL.playerNames[i].Gamertag, CGL.playerNames[i].plrRep, CGL.playerNames[i].isLegend, CGL.playerNames[i].isDev, CGL.playerNames[i].isPunisher, false, false);
