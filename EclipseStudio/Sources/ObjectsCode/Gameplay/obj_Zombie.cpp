...
void obj_Zombie::StartWalkAnim(bool run)
{
	...
	if(!run && !CreateParams.FastZombie) {
		switch(u_random(3)) {
		case 0:	aid = AddAnimation("Zombie_Walk_02"); break;
		case 1:	aid = AddAnimation("Zombie_Walk_LegDrag"); break;
		case 2:	aid = AddAnimation("Zombie_Walk_Wander"); break;
		wsk = .8f; //2.2f; 
		}
	} else if(!run && CreateParams.FastZombie) {
		switch(u_random(2)) {
		case 0:	aid = AddAnimation("Zombie_Walk_01"); break;
		case 1:	aid = AddAnimation("Zombie_Walk_04"); break;
		wsk = 1.0f;
		}
	} else if(run && !CreateParams.FastZombie) {
		...
		case 1:	aid = AddAnimation("Zombie_Sprint_01"); break;
		case 2:	aid = AddAnimation("Zombie_Sprint_02"); break;
		case 3:	aid = AddAnimation("Zombie_Run_04"); break;
		wsk = 0.5f; //0.3f;
		}
...
void obj_Zombie::SetZombieState(int state, bool atStart)
{
	...
	switch(ZombieState)
	{
		...
		case EZombieStates::ZState_Sleep:
		{
			...
			switch(u_random(4)) {
				default:
				// originais Zombie_Dead_Stand_B_01 -- Zombie_Dead_Stand_F_01
				case 0:	aid = AddAnimation("Zombie_Dead_Stand_B_01"); break;
				case 1:	aid = AddAnimation("Zombie_Dead_Stand_F_01"); break;
				case 2:	aid = AddAnimation("Zombie_Dead_Stand_B_03"); break;
				case 3:	aid = AddAnimation("Zombie_Dead_Stand_B_04"); break;
			}
		...
		case EZombieStates::ZState_Idle:
		{
			...
			switch(u_random(4)) {
				...
				case 3: aid = AddAnimation("Zombie_Idle_04"); break;	
			}
			if(u_random(10) == 0)
				//aid = AddAnimation("Zombie_Eat_Crouched_01");
				switch(u_random(2)) {
					case 0: aid = AddAnimation("Zombie_Eat_Crouched_01"); break;
					case 1: aid = AddAnimation("Zombie_Eat_Crouched_03"); break;
					}
		...
		case EZombieStates::ZState_BarricadeAttack:
			...
			switch(u_random(5)) {
				...
				case 3:	aid = AddAnimation("Zombie_Sprint_01_Hit"); break;
				case 4:	aid = AddAnimation("Zombie_Bite_Attack_1"); break;
			}
