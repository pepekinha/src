...
static bool checkIfAlreadyRunning()
{
  ...
  if((h = OpenEvent(SYNCHRONIZE | EVENT_MODIFY_STATE, FALSE, gameName)) != NULL) {
    MessageBox(NULL, "Ready to play!", "Warning", MB_OK | MB_ICONEXCLAMATION);
    CloseHandle(h);
    return true;
  }
...
{ 
  ...
  if(!testForWriteToDir()) {
    MessageBox(NULL, "Execute o jogo em modo administrador.", "UAC problem", MB_OK | MB_ICONINFORMATION);
    TerminateProcess(GetCurrentProcess(), 0);
    return;
  }
...
static void drawSerialCheckButtons(CUpdater& updater)
{
  ...
  // some temp warning
  {
    ...
    g_font1->PrintF(317, 325, clr, "Clique em obter serial e pegue seu serial.");
...
static void drawLoginButtons(CUpdater& updater)
{
  ...
  if(drawUrl(481, 445, "Perdeu a Senha?")) {
  ...
  if(insideRect(g_rRegisterNewBtn, g_mx, g_my)) 
  {
	...
	if(g_mb) {
		...
		ShellExecute(NULL, "open", "http://viruzmmo.com", "", NULL, SW_SHOW);
...
static void drawServerStatus(const CUpdater& updater)
{
  ...
  g_font1->PrintF(610, y, r3dColor(153, 208, 237), "Status:");
...
*/  
  ...
  */
  ...
  sprintf(GAME_START_PARAM, "-svchost %s -WOLogin \"%s\"", __r3dCmdLine, token);
